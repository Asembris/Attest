...
| **Full pipeline**, prose in, 40 cases | accuracy **1.0**, macro-F1 **1.0**, **40/40** extractions exact with **0** extras and **0** duplicates, **39/40** model-authored explanations, **1** template fallback, **5** guard rejections, **$0.0153102**, pass@3 = 1.0 | [`full.json`](benchmark/results/full.json) · `just bench-full` |
...
| **External trial** — a catalog we did **not** author, **not scored** | 15 claims, **14/15** outcomes matched the pre-run reading and **14/15** answered the question asked — the same fourteen. The 15th is the case that used to be right by luck; a family guard now refuses it as `No-Claim`, so the match count **fell** 15 → 14 while nothing got worse. **14/15** URNs verbatim, **0** template fallbacks, **$0.0077** — and **0 of 67 datasets unauditable**, measured both ways | [`results-after-corpgroup.json`](docs/external-trial/results-after-corpgroup.json) · [`census-after-corpgroup.json`](docs/external-trial/census-after-corpgroup.json) · `just external-trial` |
...
gap our own seed could never expose: **15 of those 67 datasets were unauditable**, because Attest's
GraphQL query had no `CorpGroup` arm and the seed only ever emits `CorpUser` owners. The guard
failed *closed*, which is correct, with a *wrong diagnosis*, which is not.

**That gap is now closed, and closed seed-first** — a group-owned dataset was seeded *before* the
`... on CorpGroup` arm was added, so the fix does not rest on the same seed that hid the problem.
The close is measured rather than asserted: a two-arm census over one loaded catalog state reads
**67/67 readable with the arm and 52/67 without it**, 15 recovered, 0 still refused, 0 regressed.
That row is not a score and is deliberately not scored:
[docs/external-trial.md](docs/external-trial.md).
...
| [docs/external-trial.md](docs/external-trial.md) | Attest against a catalog it did **not** author: 15 claims over DataHub's showcase datapack, the `CorpGroup` gap the seed could never expose — found, then closed seed-first and re-measured — and what one small external catalog does not prove. |
...
| [docs/mcp-evaluation.md](docs/mcp-evaluation.md) | Engaging the MCP Server: the adapter, the 17-dataset parity run, the verdict table, and why the verdict read stays on GraphQL. |
